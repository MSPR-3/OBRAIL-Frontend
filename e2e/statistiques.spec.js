...
test('affiche le graphique Jour vs Nuit', async ({ page }) => {
  const chartCard = page.locator('.chart-card').filter({ has: page.locator('.chart-card-title', { hasText: 'Jour vs Nuit' }) });
...
test('affiche le graphique Top 10 lignes', async ({ page }) => {
  ...
  await expect(
    page.locator('svg text').filter({ hasText: 'ThalysParis-Bruxelles' })
