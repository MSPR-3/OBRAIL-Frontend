...
export function Icon({ name, size = 18 }) {
  ...
  const paths = {
    ...
    train: <><rect x="5" y="3" width="14" height="14" rx="3"/><path d="M5 11h14"/><circle cx="9" cy="14" r=".8" fill="currentColor"/><circle cx="15" cy="14" r=".8" fill="currentColor"/><path d="M7 17l-2 4M17 17l2 4"/></>,
    chart: <><path d="M3 21h18"/><rect x="5" y="11" width="3" height="9"/><rect x="11" y="6" width="3" height="14"/><rect x="17" y="14" width="3" height="6"/></>,
    ...
    leaf: <><path d="M11 20A7 7 0 0 1 4 13c0-5 4-9 16-9 0 12-4 16-9 16z"/><path d="M4 20l8-8"/></>,
    clock: <><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></>,
    server: <><rect x="3" y="4" width="18" height="7" rx="1"/><rect x="3" y="13" width="18" height="7" rx="1"/><circle cx="7" cy="7.5" r=".8" fill="currentColor"/><circle cx="7" cy="16.5" r=".8" fill="currentColor"/></>,
    ...
    <header className="header">
      <div className="header-left">
        <div className="breadcrumb">
          ...
          {breadcrumb.map((b, i) => (
            <span key={i} style={{ color: i === breadcrumb.length - 1 ? 'var(--text-secondary)' : undefined }}>{b}</span>
          ))}
        </div>
...
export function Badge({ tone, children, role }) {
  return <span className="badge" data-tone={tone} role={role}>{children}</span>;
}
...
export function StatutBadge({ statut }) {
  ...
  return <Badge tone={cfg.tone} role="status">{cfg.label}</Badge>;
}

export function TypeBadge({ type }) {
  return <Badge tone={type === 'jour' ? 'day' : 'night'}>
    {type === 'jour' ? '☼ Jour' : '☾ Nuit'}
  </Badge>;
}