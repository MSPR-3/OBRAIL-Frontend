...
export function InfoCell({ label, value, sub, accent }) {
  return (
    <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 6, padding: 12 }}>
      <div style={{ fontSize: 10, fontFamily: 'var(--font-display)', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--text-tertiary)', fontWeight: 600 }}>
        {label}
      </div>
      <div style={{ fontSize: 15, fontWeight: 600, marginTop: 4, color: accent === 'success' ? 'var(--success)' : 'var(--text-primary)' }}>
        {value}
      </div>
      {sub && <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2, fontFamily: 'var(--font-mono)' }}>{sub}</div>}
    </div>
  );
}
...
export function TrajetDrawer({ trajet, onClose }) {
  ...
  return (
    <>
      ...
      <aside className="drawer" data-open={open} role="dialog" aria-modal="true" aria-label="Détail du trajet">
        {trajet && (
          <>
            ...
            <div className="drawer-body">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
                <InfoCell label="Départ" value={trajet.dep} sub={`Quai ${trajet.quaiD} · ${trajet.heure}`} />
                ...
                <InfoCell label="CO₂ économisé vs avion" value={`${trajet.co2} kg`} accent="success" />
                <InfoCell label="Opérateur" value={trajet.op} />
                <InfoCell label="N° de train" value={trajet.train} />
              </div>
              <div className="section-title" style={{ marginBottom: 12 }}>Chronologie du trajet</div>
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--text-secondary)' }}>
        ...
        <select className="select" style={{ width: 70 }} value={perPage} onChange={e => onPerPage(Number(e.target.value))}>
          <option>10</option><option>15</option><option>25</option><option>50</option>
        </select>
      </div>
      <div style={{ display: 'flex', gap: 4 }}>
        <button className="btn" data-size="sm" disabled={page === 1} onClick={() => onPage(page - 1)} aria-label="Page précédente">‹ Précédent</button>
        {visible.map((p, i) => p === '...'
          ? <span key={i} style={{ padding: '5px 10px', color: 'var(--text-tertiary)' }}>…</span>
          : <button key={i} className="btn" data-size="sm" data-variant={p === page ? 'primary' : ''} onClick={() => onPage(p)} aria-current={p === page ? 'page' : undefined} aria-label={`Page ${p}`}>{p}</button>
        )}
        <button className="btn" data-size="sm" disabled={page === pages} onClick={() => onPage(page + 1)} aria-label="Page suivante">Suivant ›</button>
      </div>
    </div>
  );
}
