...
export default function Statistiques() {
  ...
  const topLignes = useMemo(
    () => (volLignes?.repartition ?? []).slice(0, 10).map(r => ({ name: r.nom_ligne, nb_trajets: r.trajets })),
    [volLignes],
  );
  ...
  if (loading) return <div className="page" style={{ padding: 32, color: 'var(--text-tertiary)' }}>Chargement…</div>;
  if (!kpi)    return <div className="page" style={{ padding: 32 }}>Aucune donnée.</div>;
    ...
    <div className="page">
      <section className="page-section">
        <div className="grid-4">
          ...
          <MetricCard label="Lignes actives"      value={kpi?.total_lignes ?? '—'}                  sub={`${kpi?.total_gares ?? '—'} gares · ${kpi?.total_pays ?? '—'} pays`} icon="train" accent="true" />
        </div>
      </section>
      <section className="page-section">
          ...
          <div className="chart-card">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around', fontSize: 12, paddingTop: 8, borderTop: '1px solid var(--border)' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, marginTop: 4 }}>{kpi?.trajets_jour ?? 0}</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, color: 'var(--text-tertiary)', fontFamily: 'var(--font-display)', letterSpacing: '0.12em', textTransform: 'uppercase', fontSize: 10, fontWeight: 600 }}>
                  <span style={{ width: 8, height: 8, background: '#6FA3DB', borderRadius: 2 }} />Nuit
                </div>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, marginTop: 4 }}>{kpi?.trajets_nuit ?? 0}</div>
              </div>
            </div>
          </div>
            ...
            <div style={{ height: 280 }}>
              <ResponsiveContainer>
                <BarChart data={opData} margin={{ top: 10, right: 10, bottom: 10, left: 0 }}>
                  ...
                  <Bar yAxisId="right" dataKey="co2"     name="CO₂ moy (kg)" fill="var(--success)" radius={[4, 4, 0, 0]} opacity={0.5} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <section className="page-section">
        <div className="grid-2">
          <div className="chart-card">
            ...
            <div style={{ height: 280 }}>
              <ResponsiveContainer>
                <BarChart data={topLignes} layout="vertical" margin={{ top: 5, right: 10, bottom: 5, left: 120 }}>
      ...
      <section className="page-section">
        <div className="panel">
          ...
          <table className="table">
            <thead>
              <tr><th>Indicateur</th><th className="num" style={{ color: 'var(--accent)' }}>☼ Jour</th><th className="num" style={{ color: '#6FA3DB' }}>☾ Nuit</th><th>Unité</th></tr>
            </thead>
            <tbody>
