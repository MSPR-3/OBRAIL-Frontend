...
function Spinner() {
  return <div style={{ padding: 32, color: 'var(--text-tertiary)', textAlign: 'center' }}>Chargement…</div>;
}
    ...
    <div className="page">
      <section className="page-section">
        ...
        <div className="grid-4">
          ...
          <MetricCard label="Opérateurs"     value={nbOps}    sub={`${nbLignes} lignes · ${nbGares} gares`} icon="server" accent="success" />
          <MetricCard label="CO₂ total"      value={`${co2Total.toLocaleString('fr-FR')} kg`} sub={`${co2Moy} kg / trajet`} icon="leaf" accent="warning" />
          <MetricCard label="Pays desservis" value={dash?.total_pays ?? 0} sub="dans localisation" icon="map" accent="secondary" />
        </div>
      </section>
      ...
      <section className="page-section">
        <div className="grid-2-12">
          <div className="metric-card" data-accent="true">
            <div className="metric-head"><span>CO₂ par opérateur</span><span className="metric-icon"><Icon name="leaf" size={16} /></span></div>
            <div style={{ borderTop: '1px solid var(--border)', marginTop: 14, paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {(co2Stats?.par_operateur ?? []).slice(0, 5).map((e, i) => (
                <div key={e.id_operateur ?? i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 8, height: 8, borderRadius: 2, background: opColor(i) }} />
                    {e.nom ?? '—'}
                  </span>
          ...
          <div className="panel">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {importsResp?.imports?.map(imp => (
                <div key={imp.id_import} style={{ padding: 12, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-tertiary)' }}>
                      #{imp.id_import} · {new Date(imp.date_import).toLocaleString('fr-FR')}
                    </span>
                    <Badge tone={imp.statut === 'succès' ? 'success' : imp.statut === 'échec' ? 'danger' : 'warning'}>
                      {imp.statut}
                    </Badge>
                  </div>
