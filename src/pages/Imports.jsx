...
export default function Imports() {
  ...
  if (error)   return <div className="page" style={{ padding: 32, color: 'var(--danger)' }}>Erreur : {error}</div>;

  const list = all?.imports ?? [];

  if (!list || list.length === 0) return <div className="page" style={{ padding: 32 }}>Aucun import.</div>;
    ...
    <div className="page">
      ...
      <section className="page-section">
        <div className="grid-4">
          <MetricCard label="Total imports"    value={stats?.total_imports ?? 0}                                                                        sub="historique_import"                                                                                   icon="refresh" accent="true"    />
          <MetricCard label="Réussis"          value={stats?.imports_reussis ?? 0}                                                                      sub={stats?.total_imports ? `${Math.round((stats.imports_reussis / stats.total_imports) * 100)}% du total` : '—'} icon="server"  accent="success" />
          <MetricCard label="Échoués"          value={stats?.imports_echoues ?? 0}                                                                      sub={stats?.total_imports ? `${Math.round((stats.imports_echoues / stats.total_imports) * 100)}% du total` : '—'} icon="server"  accent="danger"  />
          <MetricCard label="Lignes importées" value={(stats?.lignes_importees_total ?? 0).toLocaleString('fr-FR')}                                     sub="cumulé"                                                                                              icon="chart"                    />
        </div>
      </section>
